...
function orgNodeHTML(person, extra = '') {
  ...
  // Show short name (before bin/binti) on the card; full name lives inside the modal via mData
  const displayName = person.name.split(/\s+bin(?:ti)?\s+/i)[0].trim() || person.name;
  return `
